...
public final class VClientImpl extends IVClient.Stub {
    ...
    public boolean CopyFileToFilesDir(String filePath, String outPath) {
        try {
            File hFile = new File(filePath);
            if(!hFile.exists()) { return false;}
            ...
            Log.e("INJECT", "CopyFileToFilesDir err: " + e.getMessage());
            return false;
        }
        return true;
    }
    ...
    private void bindApplicationNoCheck(String packageName, String processName, ConditionVariable lock) {
        Log.e("INJECT", "bindApplicationNoCheck packageName = " + packageName);
        ...
        // 加载自己的so
        String outFileSoPath = context.getFilesDir().getAbsolutePath() + "/libtest.so";
        File newFile = new File(outFileSoPath);
        if(newFile.exists()) { newFile.delete();}
        if(CopyFileToFilesDir("/data/local/tmp/libtest.so", outFileSoPath)) {
            System.load(outFileSoPath);
        }
